...
@section('content')
    ...
    <!-----start carousel --->
    <div  class=" relative " >
        <div class="d-block w-100 h"  style=" background: linear-gradient(to top, #4321daba, #2b10c7cc), url({{url('/storage/'.$category->background_img)}}) no-repeat top center;"        >
            {{-- {{url('/storage/'.$category->background_img)}} --}}
        </div>
        <div class="abs w-100">
            <h4 class="custom-h4">{!! $category['name_'.app()->getLocale()] !!}</h4>
            <h1>{!! $category['title_'.app()->getLocale()] !!}</h1>
            <h2>{!! $category['content_'.app()->getLocale()] !!}</h2>
        </div>
    </div>
    <!--- end head --->
    ...
    <div id="carouselExampleIndicators" class="carousel slide relative" data-ride="carousel">
        ...
        <div class="carousel-inner">
            @php
                $i = 0;
            @endphp
            @foreach ($category_images as $one)
...
<div class="container-fluid mb-5">
    <br>
    {!! $category['section1_'.app()->getLocale()] !!}
    ...
    <div class="container-fluid custom-container">
        <div class="row justify-content-between">
            <div class="col-lg-4 col-md-6 col-12 mt-sm-3">
                <br>
            <h4 class="custom-h4 pl-0 pr-0">{!!$category['section2_title_'.app()->getLocale()] !!}
            </h4>
<br>
                <h1>{!!$category['section2_content_'.app()->getLocale()] !!}</h1>

            </div>
            <div class="col-lg-6 col-md-6 col-12 ">
                <img src="{{url('/storage/'.$category->section2_img)}}" class="w-100 img-modal" alt="">
            </div>
        </div>
    </div>
